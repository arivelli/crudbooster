...
$namespace = '\arivelli\crudbooster\controllers';

Route::group(['middleware' => ['api', '\arivelli\crudbooster\middlewares\CBAuthAPI'], 'namespace' => 'App\Http\Controllers'], function () {
...
Route::group([
    ...
    if (Request::is(config('crudbooster.ADMIN_PATH'))) {
        ...
        if (! $menus) {
            CRUDBooster::routeController('/', 'AdminController', $namespace = '\arivelli\crudbooster\controllers');
        }
    }

    CRUDBooster::routeController('api_generator', 'ApiCustomController', $namespace = '\arivelli\crudbooster\controllers');
    ...
    try {
        ...
        foreach ($moduls as $v) {
            if (@$v->path && @$v->controller) {
                CRUDBooster::routeController($v->path, $v->controller, $namespace = '\arivelli\crudbooster\controllers');
            }
        }
    } catch (Exception $e) {
